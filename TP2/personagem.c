...
typedef struct {
    char elementos[MAX_NOMES][MAX_NAME_LENGTH];
    int tamanho;
} Lista;
...
void ler(Personagem *personagem, char *line) {
    ...
    while (alternativeNames_token != NULL && i < MAX_NOMES) {
    ...
    while (alternativeActors_token != NULL && i < MAX_ATORES) {
        // Remover colchetes e todas as aspas simples
